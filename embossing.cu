...
int main(int argc, char *argv[]){
	...
	cudaErrorIdentifier = cudaMalloc(&mA_src,vect.size()*sizeof(unsigned char));
	if (cudaErrorIdentifier != cudaSuccess)
		cout<<"Erreur à l'allocation de mA_src"<<endl;
		
	cudaErrorIdentifier = cudaMemcpy(mA_src,rgb,vect.size()*sizeof(unsigned char),cudaMemcpyHostToDevice);
	if (cudaErrorIdentifier != cudaSuccess)
		cout<<"Erreur à l'envoi des données à mA_src"<<endl;
	...
	embossing<<<grid,block>>>(mA_dest,mA_src,m_in.cols,m_in.rows);
	...
	cudaErrorIdentifier = cudaMemcpy(vect.data(),mA_dest,vect.size()*sizeof(unsigned char),cudaMemcpyDeviceToHost);
	if (cudaErrorIdentifier != cudaSuccess)
		cout<<"Erreur à la réception des données de mA_dest"<<endl;
		
	std::string path(argv[1]);
	imwrite( "out_embossing_cu_"+path, m_out );
